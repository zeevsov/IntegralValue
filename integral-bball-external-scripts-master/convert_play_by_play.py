...
team_info = pd.read_csv("{}-teams.csv".format(LEAGUE_NAME.lower().replace(" ", "-")), index_col=None, error_bad_lines=False).rename(columns={
    "TEAM": "SHORT",
    "HOME_TEAM": "FULL",
})
df["TEAM"] = df["TEAM"].map(team_info.set_index("SHORT")["FULL"])

# Creating a dataframe of unique players. Will contain name and team columns, and an index which will be the id.
unique_players = pd.concat([pd.DataFrame(df.HOME_LINEUP.str.split("/").tolist(), index=df.HOME_TEAM), pd.DataFrame(df.AWAY_LINEUP.str.split("/").tolist(), index=df.AWAY_TEAM)])
...
for i, d in qtr_starts.iterrows():
    df.ix[i, "LEN_OF_PLAY"] = qtr_first_action(d)
